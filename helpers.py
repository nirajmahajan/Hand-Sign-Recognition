...
def loadDataset():
	...
	with h5py.File(filename, 'r') as f:
		...
		data_train_x = np.array(f['train_set_x'])
		data_train_x = data_train_x.reshape(data_train_x.shape[0], -1).T/255.

		# Convert the labels to one_hot
		data_train_y = np.array(f['train_set_y'])
		data_train_y = to_one_hot(data_train_y, 6)
	...
	with h5py.File(filename, 'r') as f:
		# Get the data - flatten and normalize it
		data_test_x = np.array(f['test_set_x'])
		data_test_x = data_test_x.reshape(data_test_x.shape[0], -1).T/255.

		# Convert the labels to one_hot
		data_test_y = np.array(f['test_set_y'])
		data_test_y = to_one_hot(data_test_y, 6)
